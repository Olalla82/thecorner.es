...
while ($row = mysqli_fetch_assoc($res)) {
    ...
    $nombre_curso = !empty($row['nom_curs']) ? trim($row['nom_curs']) : trim($row['nom_comercial']);
...
if (count($cursos_coincidentes) === 0) {
    ...
    while ($grupo_row = mysqli_fetch_assoc($res_grupos_consorci)) {
    ...
    if (count($cursos_coincidentes) === 0) {
        ...
        while ($grupo_row = mysqli_fetch_assoc($res_grupos_foap)) {
...
$nombre = !empty($curso['nom_curs']) ? trim($curso['nom_curs']) : trim($curso['nom_comercial']);
...
if ($grupo_foap_encontrado) {
  ...
  $primer_grup = mysqli_fetch_assoc($result_grup);
...
elseif ($es_curso_completo) {
  ...
  $primer_grupo_data = mysqli_fetch_assoc($result_primer_grupo);
  ...
  if ($primer_grup) {
    ...
    $horari_data = mysqli_fetch_assoc($result_horaris);
...
if ($primer_grup && $primer_grup['professor']) {
    ...
    $profe_data = mysqli_fetch_assoc($result_profe);
...
if ($curso['tipus_subvencionada'] == 'FOAP') {
    ...
    if ($grupo_foap_encontrado && !empty($primer_grup['nom_soc'])) {
        ...
        if ($result_mod && $mod_data = mysqli_fetch_assoc($result_mod)) {

            $curso['hores'] = $mod_data['hores'];

            $curso['objectius'] = !empty($mod_data['objectius']) ? '<div>' . nl2br($mod_data['objectius']) . '</div>' : '';

            $curso['continguts'] = !empty($mod_data['continguts']) ? nl2br($mod_data['continguts']) : '';
        ...
        if ($result_ef_img && $ef_img_data = mysqli_fetch_assoc($result_ef_img)) {
        ...
        if ($result_ef && $ef_data = mysqli_fetch_assoc($result_ef)) {
        ...
        while ($obj_data = mysqli_fetch_assoc($result_obj)) {
        ...
        while ($cont_data = mysqli_fetch_assoc($result_cont)) {
    ...
    if (!empty($primer_grup['accio_formativa'])) {
        ...
        if ($result_any_grup && $any_grup = mysqli_fetch_assoc($result_any_grup)) {
    ...
    if ($accio_formativa_id) {
        ...
        if ($result_af && $af_data = mysqli_fetch_assoc($result_af)) {
...
$grupo_con_relacionados = mysqli_fetch_assoc($result_grupo_actual);
...
foreach ($grupos_relacionados_ids as $grupo_id) {
  ...
  if ($result_grupo && $grupo_data = mysqli_fetch_assoc($result_grupo)) {
    ...
    if ($grupo_data['etapa'] == 25) { // FOAP
      ...
      if ($r_img && $img_data = mysqli_fetch_assoc($r_img)) {
