...
/*
	Generates HTML to display ACL area: table + form
*/
function crb_acl_build_table_view( string $tag, int $slice_id = 0 ) {
    global $wpdb;
    ...
    if ( $rows = $wpdb->get_results( 'SELECT * FROM ' . CERBER_ACL_TABLE . " WHERE acl_slice = '.$slice_id.' AND tag = '" . $tag . "' ORDER BY ip_long_begin, ip" ) ) {
        ...
        $ret = '<table id="acl_' . $tag . '" class="acl-table" data-acl-slice="' . $slice_id . '"><tr>' . implode( '</tr><tr>', $list ) . '</tr></table>';
    }
    else {
 ...
 *
 */
function cerber_show_activity_page( $args = array(), $echo = true ) {
...
function crb_top_malicious_ip_report() {
	...
	if ( empty( $results ) ) {
		return array( 'no_data' => __( 'No malicious activity detected in the past 24 hours.', 'wp-cerber' ) );
	}
...
function cerber_quick_w() {
	...
	echo '<td style="width:50%; vertical-align:top;"><table><tr><td class="bigdig">' . $locked . '</td><td class="per">' . $locked_ch . '</td></tr></table><p>' . __( 'lockouts in the last 24 hours', 'wp-cerber' ) . '<br/>(<a href="' . $act . '&amp;filter_activity[]=10&filter_activity[]=11">' . __( 'view all', 'wp-cerber' ) . '</a>)</p></td></tr></table>';
...
*/
function cerber_show_help_page() {
...
function cerber_widgets_init() {
	...
	CRB_Widgets::register( 'wgt_new_users',
		...
		function () {
			...
			return cerber_show_activity_page( array(
