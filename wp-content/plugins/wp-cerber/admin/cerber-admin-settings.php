...
function cerber_settings_config( $args = array() ) {
	...
	if ( ! cerber_is_permalink_enabled() ) {
		...
		$custom = __( 'Be careful about enabling these options. If you forget your Custom login URL, you will be unable to log in.', 'wp-cerber' );
	}
	...
	if ( ! $sections ) {
		...
		$sections = array(
			...
			'scanexcl'    => array(
				...
				'desc'   => __( 'These files will never be deleted during automatic cleanup. Be careful about configuring these settings. Improper configuration may lead to failure to delete malicious files.', 'wp-cerber' ),
				'fields' => array(
					'scan_delexdir'  => array(
			...
			'antibot_more' => array(
				...
				'fields' => array(
					...
					'botswhite'        => array(
						...
						'label'     => __( 'Specify any part of a query string or path to exclude matching requests from inspection by the anti-spam engine. One exception per line. To specify a REGEX pattern, enclose the exception in two braces.', 'wp-cerber' ),
						'type'      => 'textarea',
						'list'      => true,
						'delimiter' => "\n",
