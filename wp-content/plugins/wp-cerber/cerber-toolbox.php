...
function cerber_issue_monitor() {
	...
	if ( version_compare( CERBER_REQ_PHP, phpversion(), '>' ) ) {
		...
		$notices['php'] = sprintf( __( 'WP Cerber requires PHP version %1$s or higher, but your web server is currently running PHP %2$s.', 'wp-cerber' ), CERBER_REQ_PHP, phpversion() );
	}
	...
	if ( ! crb_wp_version_compare( CERBER_REQ_WP ) ) {
		...
		$notices['wordpress'] = sprintf( __( 'WP Cerber requires WordPress version %1$s or higher. Your WordPress version is %2$s. Please update your WordPress to the latest version.', 'wp-cerber' ), CERBER_REQ_WP, cerber_get_wp_version() );
	}
