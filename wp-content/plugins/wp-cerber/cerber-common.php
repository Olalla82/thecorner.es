...
function cerber_add_issue( $code, $text, $data = array() ) {
	...
	// temp workaround for staging

	$issues = cerber_get_set( 'temp_issues' );
	...
	cerber_update_set( 'temp_issues', $issues );

	return;

	// TODO implement viewing/erasing
...
function cerber_check_for_update( $update, $plugin_data, $plugin_file, $locales ) {
	...
	$release_info['translations'] = crb_process_locales( $locales, $available );
