...
function crb_check_ownership( int &$stage_progress, int $num = 2 ) {
	...
	return CRB_Scan::iterate_sections_stateful( 3, function ( $section ) {
		...
		$msg = sprintf( __( 'At %1$s the scanner detected that the ownership of the plugin changed from %2$s to %3$s.', 'wp-cerber' ), $last_date, '<a href="' . $prev_profile['profile'] . '" target="_blank">' . $prev_profile['display_name'] . '</a>', '<a href="' . $new_profile['profile'] . '" target="_blank">' . $new_profile['display_name'] . '</a>' );
